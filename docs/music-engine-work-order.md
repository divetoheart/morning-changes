...
A single fret may belong to multiple active layers. The previous approach placed multiple labeled dots in the same cell and created contradictory-looking interval labels. The renderer now resolves each string/fret to one primary label.
...
2. Other active layers use a compact multi-layer marker treatment, never a second competing interval label:
   ...
   - title/ARIA detail currently lists every active role and interval;
   - later: tap/click detail panel for touch devices.
3. When two layers agree on the interval, show the shared label once and add the multi-layer treatment.
4. When they disagree, show the focus-layer label and expose the other role in the detail text.
...
- [x] Add development music-engine contract suite for spelling, chord formulas, tuning order, root coordinates, all 12 major-key roots, collision resolution, shell voicings, and Drop 2 voice order.
...
- [x] Contract cases for all 12 major roots plus critical flat/sharp, diminished, augmented, half-diminished, Drop 2, and shell cases.
...
- [ ] Add a tap/click per-fret detail panel with every active membership.
...
- [ ] Layer membership is tap/click inspectable on touch devices.
