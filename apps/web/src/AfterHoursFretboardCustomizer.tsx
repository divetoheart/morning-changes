...
export function AfterHoursFretboardCustomizer({ keyLabel, majorRoot, minorRoot, chords, description, cagedLabel, pentatonicLabel }: Props) {
  const [enabled, setEnabled] = useState<Record<Layer, boolean>>({ caged:true, pentatonic:true, arpeggio:true, scale:false });
  ...
  return <section className="ah-fretboard-customizer">
    <div className="ah-piece-section-head">
      <div><span className="eyebrow">Shapes and voicings</span><h2>One neck. Every map you need.</h2><p>{description}</p></div>
      <div className="ah-fretboard-key"><small>Study key</small><strong>{keyLabel}</strong></div>
    </div>
    <div className="ah-fretboard-controls">
      <label className="ah-fretboard-chord"><span>Active chord</span><select value={activeChordLabel} onChange={event => setActiveChordLabel(event.target.value)}>{chords.map(chord => <option key={chord.label} value={chord.label}>{chord.label}</option>)}</select></label>
      <div className="ah-fretboard-layers" aria-label="Fretboard layers">{LAYERS.map(layer => <button type="button" key={layer.id} className={`layer-${layer.id} ${enabled[layer.id] ? 'active' : ''}`} aria-pressed={enabled[layer.id]} onClick={() => setEnabled(previous => ({ ...previous, [layer.id]: !previous[layer.id] }))}><i aria-hidden="true"/><span>{layer.label}</span><small>{layer.id === 'caged' ? cagedLabel : layer.id === 'pentatonic' ? pentatonicLabel : layer.detail}</small></button>)}</div>
    </div>
    <div className="ah-fretboard-legend"><span><b className="root-key">1</b> Root notes use dark ink on a light marker.</span><span>High <b>e</b> is on top. Low <b>E</b> is on bottom.</span><span>0–15 frets · first repeated position after 12th fret included.</span></div>
    <div className="ah-full-neck-scroll" tabIndex={0} role="region" aria-label="Full fretboard with selectable study layers">
      <div className="ah-full-neck">
        <div className="ah-full-neck-frets"><span></span>{frets.map(fret => <span key={fret}>{fret}</span>)}</div>
        {DISPLAY_STRINGS.map(string => <div className="ah-full-neck-string" key={string.label}><b>{string.label}</b>{frets.map(fret => <span className="ah-full-neck-cell" key={fret}>{dotsFor(string, fret).map((dot, index) => <i key={`${dot.layer}-${index}`} className={`ah-layer-dot ${dot.layer} ${dot.root ? 'root' : ''}`} title={`${dot.layer}: ${dot.label}`}><small>{dot.label}</small></i>)}</span>)}</div>)}
      </div>
    </div>
  </section>;
}
