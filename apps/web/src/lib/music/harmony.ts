...
import type { Chord, ChordQuality, FunctionalChord, KeyContext, RomanDegree, ScaleMode, SpelledNote, Triad, TriadQuality, TriadTone } from './types';
...
export function triadForChord(chord: Chord): Triad | undefined { const quality = triadQualityForChord(chord.quality); return quality ? buildTriad(chord.root, quality) : undefined; }
export function chordSymbol(chord: Chord): string { return `${noteToString(chord.root)}${chordQualitySuffix(chord.quality)}`; }
export function buildFunctionalChord(key: KeyContext, functional: FunctionalChord): Chord { return buildChord(degreeNote(key, functional.degree), functional.quality); }
