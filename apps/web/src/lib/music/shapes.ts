...
/**
 * Standard-tuning minor-pentatonic geometries, expressed relative to a root on
 * low E. The G-to-B tuning offset is encoded in the parent positions.
 */
...
export function generateMinorPentatonicCycle(
  ...
  const rootNote = typeof root === 'string' ? parseNote(root) : root;
  return rootAnchorsForRange(rootNote, tuning, range).flatMap(rootAnchorFret =>
    MINOR_PENTATONIC_BOX_ORDER.map(box => generateMinorPentatonicBox(box, rootNote, rootAnchorFret, tuning, range))
  );
}
