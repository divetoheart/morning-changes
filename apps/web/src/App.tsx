...
import type { HarmonyExample, Lesson, LearningPath, MusicInterval } from './domain/content';
...
const navItems: NavItem[] = [
  ...
  { to: '/paths', label: 'Paths', icon: 'paths' },
...
function AppLayout() {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => setCompletionNotice(null), 6000);
  ...
  const openLesson = (lesson: Lesson) => {
    if (lesson.access === 'premium') return setOverlay({ kind: 'premium', title: lesson.title });
  ...
  const completeLesson = (lesson: Lesson, daily = false) => {
    const path = paths.find(candidate => candidate.lessonIds.includes(lesson.id));
    const completedInPath = path ? path.lessonIds.filter(id => id === lesson.id || nextStatus(progress, id) === 'completed').length : 0;
    const index = path?.lessonIds.indexOf(lesson.id) ?? -1;
    const nextLesson = index >= 0 ? lessonById(path?.lessonIds[index + 1] ?? '') : undefined;
    setProgress(previous => markCompleted(previous, lesson, daily ? 'daily' : 'lesson'));
    setCompletionNotice({
      minutes: lesson.durationMinutes,
      progressLabel: path ? `${path.title}: ${completedInPath}/${path.lessonIds.length} complete` : 'Practice saved',
  ...
  return <div className="app-shell">
    <header className="app-topbar">
      <NavLink className="wordmark" to="/"><span className="wordmark-mark">◒</span><span><strong>Morning Changes</strong><small>Daily guitar practice</small></span></NavLink>
      <nav className="desktop-nav" aria-label="Primary navigation">{navItems.map(item => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => isActive ? 'active' : ''}><span>{item.label}</span>{item.detail && <small>{item.detail}</small>}</NavLink>)}</nav>
    ...
    <main className="screen"><Routes>
      <Route path="/" element={<HomeScreen dailyLesson={dailyLesson} dailyLick={dailyLick} dailyExercise={dailyExercise} activeLesson={activeLesson} progress={progress} onOpenDaily={() => navigate(`/lesson/${dailyLesson.id}`)} onOpenLesson={openLesson} onOpenExtra={openExtra} />} />
      <Route path="/lesson/:lessonId" element={<LessonPage progress={progress} metronome={metronome} selectedKey={selectedKey} onKeyChange={setSelectedKey} onStart={startLesson} onComplete={completeLesson} onOpenPremium={title => setOverlay({ kind: 'premium', title })} />} />
      <Route path="/practice/:kind/:extraId" element={<PracticeExtraPage metronome={metronome} onOpenLesson={id => { const lesson = lessonById(id); if (lesson) openLesson(lesson); }} />} />
    ...
    <nav className="bottom-nav" aria-label="Mobile navigation">{navItems.map(item => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => isActive ? 'active' : ''} aria-label={item.label}><Icon name={item.icon} size={19} /><span>{item.mobileLabel ?? item.label}</span></NavLink>)}</nav>
    {overlay && <PracticeOverlay overlay={overlay} progress={progress} onClose={() => setOverlay(null)} onOpenLesson={openLesson} />}
    {completionNotice && <CompletionToast notice={completionNotice} onClose={() => setCompletionNotice(null)} />}
  </div>;
}
...
function LearnScreen({ progress, activeLesson, onOpenLesson }: { progress: PracticeProgress; activeLesson: Lesson; onOpenLesson: (lesson: Lesson) => void }) {
  ...
  return <><ScreenHeader eyebrow="Learn" title="Pick a lane." copy="Lessons open as full practice pages with key-aware visuals." /><section className="continue-card"><CompactRow icon="play" label="Continue" title={activeLesson.title} detail={`${statusLabel(nextStatus(progress, activeLesson.id))} · ${activeLesson.durationMinutes} min`} action="Open" onClick={() => onOpenLesson(activeLesson)} /></section>{section('Autumn Leaves Prep', 'Root → scale → chords → guide tones → arpeggios → tune.', lessons.filter(lesson => lesson.pathIds.includes('autumn-leaves-prep')))}{section('Start here', 'Short sessions with no setup.', lessons.filter(lesson => lesson.access === 'free' && lesson.dailyEligible))}{section('Beginner', 'Intervals, roots, scale awareness, and chord building.', lessons.filter(lesson => lesson.level === 'Beginner'))}{section('Intermediate', 'Guide tones, cadences, arpeggios, and tune application.', lessons.filter(lesson => lesson.level === 'Intermediate'))}</>;
}
...
function ProgressScreen({ progress, activeLesson, onOpenLesson }: { progress: PracticeProgress; activeLesson: Lesson; onOpenLesson: (lesson: Lesson) => void }) {
  ...
  return <><ScreenHeader eyebrow="Progress" title="Your practice, kept simple." /><section className="progress-summary"><div><strong>{currentStreak(progress)}</strong><span>day streak</span></div><div><strong>{progress.history.length}</strong><span>sessions</span></div><div><strong>{complete}</strong><span>completed</span></div></section><section className="quiet-list"><span className="list-heading">Keep close</span><CompactRow icon="progress" label="Review" title="Autumn Leaves Prep" detail="Return to the current path." action="Open" onClick={() => onOpenLesson(activeLesson)} /></section></>;
}

function LessonPage({ progress, metronome, selectedKey, onKeyChange, onStart, onComplete, onOpenPremium }: { progress: PracticeProgress; metronome: ReturnType<typeof useMetronome>; selectedKey: KeyName; onKeyChange: (key: KeyName) => void; onStart: (lesson: Lesson) => void; onComplete: (lesson: Lesson, daily?: boolean) => void; onOpenPremium: (title: string) => void }) {
  const { lessonId } = useParams();
  const navigate = useNavigate();
  const lesson = lessonById(lessonId ?? '');
  useEffect(() => { if (lesson?.defaultKeyStrategy === 'random') onKeyChange(randomKey(`${lesson.id}-${new Date().toDateString()}`)); }, [lesson?.id]);
  if (!lesson) return <><ScreenHeader eyebrow="Lesson" title="Not found." copy="This lesson is not available." /><button className="secondary-button" onClick={() => navigate('/learn')}>Back to Learn</button></>;
  if (lesson.access === 'premium') return <><ScreenHeader eyebrow="Premium" title={lesson.title} copy="This lesson will unlock with Premium." /><button className="primary-button" onClick={() => onOpenPremium(lesson.title)}>Preview Premium</button></>;
  const status = nextStatus(progress, lesson.id);
  const firstStep = lesson.routine[0];
  const start = () => { onStart(lesson); if (!metronome.playing) metronome.toggle(); };
  return <article className="lesson-page"><nav className="lesson-breadcrumb"><button type="button" onClick={() => navigate('/learn')}>← Learn</button><span>{lesson.category}</span></nav><section className="play-now"><div><span className="eyebrow">Play now · {firstStep?.minutes ?? 2} min</span><h2>{firstStep?.instruction ?? lesson.outcome}</h2><p>Start with the sound. The explanation and fretboard are here when you need them.</p></div><button className="primary-button" type="button" onClick={start}><Icon name="play" size={16} /> Start at {lesson.metronome.bpm} BPM</button></section><section className="lesson-hero"><div><span className="eyebrow">{lesson.durationMinutes} min · {statusLabel(status)}</span><h1>{lesson.title}</h1><p>{lesson.outcome}</p></div><div className="lesson-actions"><KeySelector selectedKey={selectedKey} onKeyChange={onKeyChange} /><button className="secondary-button" type="button" onClick={start}>{status === 'not-started' ? 'Begin practice' : 'Continue'}</button><button className="secondary-button" type="button" onClick={() => onComplete(lesson)}>Mark complete</button></div></section><IntervalPanel lesson={lesson} selectedKey={selectedKey} onKeyChange={onKeyChange} /><Fretboard lesson={lesson} selectedKey={selectedKey} /><section className="practice-plan"><div><span className="eyebrow">Practice plan</span><h2>Work the sound, not the shape.</h2></div><ol className="routine-list">{lesson.routine.map(step => <li key={step.instruction}><span>{step.minutes} min</span>{step.instruction}</li>)}</ol><div className="sheet-actions"><button className="secondary-button" type="button" onClick={() => { metronome.setTempo(lesson.metronome.bpm); if (!metronome.playing) metronome.toggle(); }}>{lesson.metronome.bpm} BPM</button>{lesson.concept?.afterHoursHref && <a className="after-hours-bridge" href="#/after-hours/autumn-leaves">{lesson.concept.afterHoursCta} <Icon name="arrow" size={16} /></a>}</div></section></article>;
}
...
function Fretboard({ lesson, selectedKey }: { lesson: Lesson; selectedKey: KeyName }) { const intervals = lesson.concept?.intervals ?? ['1' as MusicInterval]; const targets = renderIntervals(selectedKey, intervals); const dotFor = (open: string, fret: number) => { const note = NOTES[(NOTE_INDEX[open] + fret) % 12]; return targets.find(target => target.note === note); }; return <section className="fretboard-card"><div><span className="eyebrow">Fretboard visual</span><h2>Intervals on the neck</h2><p>Dots show the lesson intervals in {selectedKey}. Change the key and the neck redraws.</p></div><div className="fretboard-scroll"><div className="fretboard"><div className="fret-row fret-numbers"><span></span>{Array.from({ length: 13 }, (_, fret) => <span key={fret}>{fret}</span>)}</div>{TUNING.map(string => <div className="fret-row" key={string.label + string.note}><b>{string.label}</b>{Array.from({ length: 13 }, (_, fret) => { const dot = dotFor(string.note, fret); return <span className={dot ? 'has-dot' : ''} key={fret}>{dot && <i><strong>{dot.interval}</strong><small>{dot.note}</small></i>}</span>; })}</div>)}</div></div></section>; }

function PracticeOverlay({ overlay, progress, onClose, onOpenLesson }: { overlay: Exclude<Overlay, null>; progress: PracticeProgress; onClose: () => void; onOpenLesson: (lesson: Lesson) => void }) {
  const sheet = () => overlay.kind === 'premium' ? <><SheetHeader eyebrow="Morning Changes Premium" title={overlay.title ?? 'Go deeper when you are ready.'} onClose={onClose} /><p>Premium unlocks complete specialization paths, longer guided sessions, and the growing After Hours catalog.</p></> : <PathSheet path={overlay.path} progress={progress} onClose={onClose} onOpenLesson={onOpenLesson} />;
