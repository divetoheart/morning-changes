...
function AppLayout() {
  ...
  const completeLesson = (lesson: Lesson, daily = false) => {
    ...
    setCompletionNotice({ minutes: lesson.durationMinutes, progressLabel: path ? `${path.title}: ${completedInPath}/${path.lessonIds.length} complete` : 'Practice saved', nextTitle: nextLesson?.title });
  };
...
function ProgressScreen({ progress, activeLesson, onOpenLesson }: { progress: PracticeProgress; activeLesson: Lesson; onOpenLesson: (lesson: Lesson) => void }) { const complete = Object.values(progress.lessonStates).filter(status => status === 'completed').length; return <><ScreenHeader eyebrow="Progress" title="Your practice, kept simple." /><ProgressDashboard progress={progress} activeLesson={activeLesson} onOpenLesson={onOpenLesson} /><section className="progress-summary"><div><strong>{currentStreak(progress)}</strong><span>day streak</span></div><div><strong>{progress.history.length}</strong><span>sessions</span></div><div><strong>{complete}</strong><span>completed</span></div></section></>; }
